...
export default function Login() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setError('');
    alert('¡Bienvenido! (Demo - sin backend)');
  };

  return (
    <div className="min-h-screen flex items-center justify-center bg-nae-grey px-4 py-12">
      <div className="bg-white rounded-xl shadow-lg p-8 max-w-md w-full">
        <h1 className="text-2xl font-bold text-center mb-2 font-heading">Iniciar Sesión</h1>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex items-center justify-between text-sm">
            ...
            <span className="text-nae-blue cursor-pointer hover:underline">¿Olvidaste tu contraseña?</span>
          </div>
        ...
        <p className="text-center text-sm text-gray-500">
          ¿No tienes cuenta? <Link to="/register" className="text-nae-blue font-medium hover:underline">Regístrate como instalador</Link>
        </p>
      </div>
    </div>
  );
